...
void microphoneStart() {
	...
	if (player->OpenFile("wavein://src=line;volume=50;", libZPlay::sfAutodetect) == 0) {
	...
	if (player->SetWaveOutFile("output.mp3", libZPlay::sfMp3, 1) == 0) {
	...
	player->SetCallbackFunc(myCallbackFunc, (libZPlay::TCallbackMessage)libZPlay::MsgStreamNeedMoreData, NULL);
